CREATE TABLE IF NOT EXISTS `model_disease` (
    id INT PRIMARY KEY COMMENT '所配的AI诊断的信息',
    `name` VARCHAR(255) NOT NULL COMMENT '诊断疾病的名称',
    description VARCHAR(255) COMMENT '所需诊断疾病说明',
...
CREATE TABLE IF NOT EXISTS `model_line` (
    ...
    description VARCHAR(255) COMMENT '放行规则描述',
    `create_time`  datetime COMMENT '创建时间',
    `update_time`  datetime COMMENT '更新时间'
)ENGINE = InnoDB
 AUTO_INCREMENT = 46
